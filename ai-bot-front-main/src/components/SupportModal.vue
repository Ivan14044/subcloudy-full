                    ...
                    <div class="modal-content">
                            ...
                            <div class="email-form">
                                ...
                                >
                                    <span v-if="isLoading" class="button-loading">
                                        <span class="button-spinner"></span>
                                        {{ safeT('support.loading', 'Загрузка...') }}
                                    </span>
                        ...
                        <div v-else-if="step === 'chat'" class="step-container">
                            <div class="messages-wrapper" ref="messagesRef">
                                <div v-if="!messages.length && isLoading" class="loading-state">
                                    <div class="spinner"></div>
                                    <p>{{ t('support.loading') }}</p>
                                </div>
                                <div v-else-if="!messages.length" class="empty-state">
                                    <p>{{ t('support.startChat') }}</p>
                                </div>
                                <div v-else class="messages-list">
                                    <template v-for="(message, index) in messages" :key="message.id">
                                        <!-- System notification for status change -->
                                        <div v-if="index > 0 && shouldShowStatusNotification(message, index)" class="system-notification">
                                            {{ t('support.statusChangedTo', { status: t(`support.statuses.${message.ticket_status}`) }) }}
                                        </div>

                                        <div
                                            class="message-item"
                                            :class="{
                                                'message-admin': message.sender_type === 'admin',
                                                'message-client': message.sender_type === 'client'
                                            }"
                                        >
                                            <div class="message-bubble">
                                                <div v-if="message.image_url" class="message-image-wrapper">
                                                    <img :src="message.image_url" class="message-image" @click="openImage(message.image_url)" />
                                                </div>
                            ...
                            <div class="message-input-wrapper">
                                <button type="button" class="attach-button" @click="triggerFileInput" :disabled="isLoading">
                                    <svg class="attach-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <button
                                    type="button"
                                    class="send-button"
                                    :disabled="(!messageText.trim() && !selectedFile) || isLoading"
                                    @click="handleSendMessage"
                                >
                                    <span v-if="isLoading" class="button-spinner-small"></span>
                                    <svg v-else class="send-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <button
                                v-if="selectedChannel === 'telegram' && ticket"
                                type="button"
...
// Безопасная функция для получения локализованных строк с обработкой ошибок
const safeT = (key: string, fallback?: string): string => {
    try {
        const result = t(key);
        // Проверяем, что результат не является ключом (что означает, что перевод не найден)
        if (result === key && fallback) {
            return fallback;
        }
        return result;
    } catch (error) {
        console.warn(`[SupportModal] i18n error for key "${key}":`, error);
        return fallback || key;
    }
};
...
const errorMessage = computed(() => supportStore.error);
...
@media (max-width: 480px) {
    ...
    .liquid-glass-effect, .liquid-glass-tint, .liquid-glass-shine { border-radius: 0; }
}
</style>
