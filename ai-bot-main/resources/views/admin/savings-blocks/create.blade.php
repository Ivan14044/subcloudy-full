...
@section('title', 'Добавить блок экономии')

@section('content_header')
    <h1>Добавить блок экономии</h1>
@endsection
...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.savings-blocks.store') }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select name="service_id" id="service_id" class="form-control @error('service_id') is-invalid @enderror">
                                ...
                                @foreach($services as $service)
                                    <option value="{{ $service->id }}" {{ old('service_id') == $service->id ? 'selected' : '' }}>{{ $service->admin_name }}</option>
                                @endforeach
                            </select>
                            @error('service_id')
                                    ...
                                    @foreach (config('langs') as $code => $flag)
                                        <div class="tab-pane fade {{ $code == 'ru' ? 'show active' : '' }}" id="content-{{ $code }}" role="tabpanel">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="title[{{ $code }}]" class="form-control @error("title.$code") is-invalid @enderror" value="{{ old("title.$code") }}">
                                                @error("title.$code")
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="our_price[{{ $code }}]" class="form-control @error("our_price.$code") is-invalid @enderror" value="{{ old("our_price.$code") }}">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="normal_price[{{ $code }}]" class="form-control @error("normal_price.$code") is-invalid @enderror" value="{{ old("normal_price.$code") }}">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="advantage[{{ $code }}]" class="form-control @error("advantage.$code") is-invalid @enderror" value="{{ old("advantage.$code") }}">
