...
@section('title', __('admin.edit_service_account') . ' #' . $serviceAccount->id)

@section('content_header')
    <h1>{{ __('admin.edit_service_account') }} #{{ $serviceAccount->id }}</h1>
@stop
                    ...
                    <form method="POST" action="{{ route('admin.service-accounts.update', $serviceAccount) }}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="max_users" id="max_users" min="1" max="1000"
                                   ...
                                   value="{{ old('max_users', $serviceAccount->max_users) }}"
                        ...
                        <div class="form-group">
                            ...
                            @if(isset($serviceAccount->credentials['cookies']) && is_array($serviceAccount->credentials['cookies']) && count($serviceAccount->credentials['cookies']) > 0)
                                <div class="alert alert-success">
                                    ✅ Сохранено {{ count($serviceAccount->credentials['cookies']) }} cookie(s)
                                </div>
                            @endif
