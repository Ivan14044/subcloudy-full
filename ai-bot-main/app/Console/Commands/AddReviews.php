...
class AddReviews extends Command
{
    protected $signature = 'reviews:add';
    protected $description = 'Добавить реалистичные отзывы в базу данных';

    public function handle()
    {
        $reviews = $this->getReviewsData();

        $this->info('Начинаю добавление отзывов...');
        ...
        foreach ($reviews as $index => $reviewData) {
            ...
            $this->line("Добавлен отзыв от: {$reviewData['name']['ru']}");
        }

        $this->info('Успешно добавлено ' . count($reviews) . ' отзывов!');
        return 0;
    }
